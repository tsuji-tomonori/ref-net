...
### 技術的正確性と実装可能性
- [ ] SQLAlchemyモデルが最新バージョン（2.0+）の推奨パターンに従っている
- [ ] Mapped型注釈が適切に使用されている
- [ ] リレーションシップが適切に設定されている（back_populates、foreign_keys等）
- [ ] データベース接続管理クラスが適切なプール設定を持っている
- [ ] 例外処理が適切に実装されている

### 統合考慮事項
- [ ] 共通ライブラリ（shared）との統合が適切に設計されている
- [ ] 環境設定管理システムと連携している
- [ ] 他のパッケージからのインポートが可能な構造になっている
- [ ] テストでの利用を考慮した設計になっている

### 品質基準
- [ ] コーディング規約に準拠している
- [ ] 型ヒントが適切に記述されている
- [ ] Docstringが十分に記述されている
- [ ] 命名規則が一貫している
...
- [ ] CHECK制約が適切に設定されている
- [ ] 外部キー制約が適切に設定されている
- [ ] コネクションプールの設定が本番環境に適している

### 保守性とドキュメント
- [ ] モデル間の関係が明確に文書化されている
- [ ] 制約の意図が理解できる
- [ ] パフォーマンス最適化の根拠が説明されている
- [ ] 将来の拡張性が考慮されている

### データベース設計固有の観点
- [ ] 論文・著者・関係性のドメインモデルが適切に表現されている
- [ ] データの正規化が適切に行われている
- [ ] 大量データを扱う前提での設計になっている
- [ ] 処理キューの設計が実用的である
- [ ] 外部ID管理が拡張可能な設計になっている
- [ ] 研究分野・キーワードのモデリングが適切である
