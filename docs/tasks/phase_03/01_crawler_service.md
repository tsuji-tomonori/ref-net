...
### 技術的正確性と実装の妥当性
- [ ] Semantic Scholar APIクライアントが適切に実装されている（リトライメカニズム、エラーハンドリング）
- [ ] HTTPクライアント設定が適切である（タイムアウト、リダイレクト等）
- [ ] 非同期処理が適切に実装されている（async/await、適切な同期制御）
- [ ] データモデルがPydanticで適切に定義されている
- [ ] 再帰的クローリングロジックが正しく実装されている
- [ ] 優先度判定アルゴリズムが適切である
...
### セキュリティとパフォーマンス
- [ ] APIキー管理が適切である（環境変数等）
- [ ] レート制限の遵守と適切なリトライ戦略が実装されている
- [ ] メモリ使用量が適切に管理されている
- [ ] 大量データ処理時のパフォーマンスが考慮されている
- [ ] 外部API障害時のサーキットブレーカー等の対策がある
- [ ] データベーストランザクションが適切に管理されている
...
### クローラー固有の観点
- [ ] 再帰的クローリングの終了条件が適切である
- [ ] 重複データの適切な処理が実装されている
- [ ] 論文関係性データの正確性が確保されている
- [ ] ホップ数と優先度制御が適切に実装されている
- [ ] バッチ処理と単一処理のバランスが適切である
- [ ] 失敗時のリトライ戦略が適切である
