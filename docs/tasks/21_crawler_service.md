...
Semantic Scholar APIを使用して論文メタデータを収集し、引用・被引用関係を再帰的に収集するクローラーサービスを実装する。
...
- [ ] 基本的なテストケースが作成されている
- [ ] Dockerfileが作成されている
- [ ] `cd package/crawler && moon check` が正常終了する
- [ ] テストカバレッジが80%以上である

## レビュー観点

### 技術的正確性
- [ ] Semantic Scholar APIの仕様に準拠した実装である
- [ ] HTTPクライアント（httpx）の非同期処理が適切に実装されている
- [ ] tenacityによるリトライ機構が適切に設定されている
- [ ] Celeryタスクの定義とワーカー設定が正しい
- [ ] データベーストランザクション管理が適切である

### 実装可能性
- [ ] APIレート制限への対応が実装されている
- [ ] 大量データ処理に対応できるバッチ処理機能がある
- [ ] メモリ使用量を考慮した実装である
- [ ] 長時間実行タスクのタイムアウト設定が適切である

### 統合考慮事項
- [ ] データベースモデルとの整合性が保たれている
- [ ] 他のサービス（API、要約）との連携が考慮されている
- [ ] 共通ライブラリ（refnet-shared）との依存関係が適切である
- [ ] Redis（Celery）との接続が確実に行われる

### 品質基準
- [ ] エラーハンドリングが網羅的である（404、429、500等）
- [ ] ログ出力が適切で監視・デバッグに役立つ
- [ ] テストでモック化が適切に行われている
- [ ] 設定ファイルと環境変数の管理が適切である

### セキュリティ考慮事項
- [ ] APIキーが安全に管理されている
- [ ] 外部APIからの応答データの検証が行われている
- [ ] ログに機密情報が出力されないよう配慮されている
- [ ] 悪意のあるデータに対する防御が考慮されている

### パフォーマンス考慮事項
- [ ] 再帰的クローリングの深度制限が適切である
- [ ] 優先度付きキューによる効率的な処理順序制御がある
- [ ] 重複データの収集を回避する仕組みがある
- [ ] 論文関係の収集が効率的に実装されている

### 保守性
- [ ] 新しいAPIエンドポイントの追加が容易である
- [ ] クローリング戦略の変更が容易である
- [ ] エラー発生時の調査・復旧が容易である
- [ ] 処理状態の監視・管理が適切である
