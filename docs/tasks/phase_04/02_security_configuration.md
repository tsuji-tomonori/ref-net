...
### セキュリティ条件
- [ ] パスワードが適切にハッシュ化されている（bcrypt rounds >= 12）
- [ ] JWTトークンが適切に管理されている（HS256以上、15分有効期限）
- [ ] APIエンドポイントが適切に保護されている
- [ ] レート制限が効果的に動作している（100req/min per IP）
- [ ] セキュリティヘッダーが設定されている（全セキュリティヘッダー）

### 暗号化要件
- [ ] データベース接続がTLS 1.2以上で暗号化されている
- [ ] API キーとシークレットが環境変数で管理されている
- [ ] Redis接続がTLS暗号化されている
- [ ] ログファイルに機密情報が含まれていない

### 入力検証要件
- [ ] 全APIエンドポイントでPydantic入力検証が動作
- [ ] SQLインジェクション防止策が実装されている
- [ ] XSS防止策が実装されている（入力サニタイゼーション）
- [ ] ファイルアップロード検証が実装されている（PDF処理）

### 監視・アラート要件
- [ ] 認証失敗アラート（5回/分以上で通知）
- [ ] 異常なトラフィックパターン検知（通常の3倍以上）
- [ ] セキュリティイベントログ記録（認証・認可・エラー）
- [ ] 脆弱性スキャン結果が定期的に確認されている
...
- [ ] セキュリティヘッダーのテストが成功
- [ ] ペネトレーションテストが実施されている
- [ ] SQLインジェクション・XSSテストが実施されている

## レビュー観点

### セキュリティ設計
- [ ] JWT実装が安全で、適切な有効期限が設定されている
...
### アクセス制御
- [ ] APIエンドポイントが適切に保護されている
- [ ] ロールベースアクセス制御（RBAC）が正しく機能している
- [ ] 権限チェックが漏れなく実装されている
- [ ] 最小権限の原則が守られている

### 通信セキュリティ
- [ ] HTTPS/TLSが正しく設定されている
- [ ] 証明書管理の仕組みが確立されている
- [ ] 安全でないプロトコルが無効化されている
- [ ] CORS設定が適切である
